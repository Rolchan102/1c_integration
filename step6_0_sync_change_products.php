 ...
 * 
 * Запуск: по cron каждые 30 секунд
 */
...
$itemsFrom1C = $oneC->getAllItems('0');
...
foreach ($itemsFrom1C as $index => $item1C) {
    try {
        ...
        if ($existingProduct) {
            ...
            if ($result) {
                $updated++;
        		confirmItemProcessed($recordId, $oneC, $config);
            } else {
                $errors++;
...
/**
 * Отправляет подтверждение обработки записи в 1С
 * @param string $recordId ИДЗаписи из 1С
 */
function confirmItemProcessed($recordId, $oneC, $config) {
    if (empty($recordId)) {
        Logger::warning('Не указан ИДЗаписи для подтверждения', ['recordId' => $recordId]);
        return false;
    }
    ...
    if (empty($login) || empty($password)) {
        Logger::error('Не настроены учётные данные 1С', ['url' => $acceptUrl]);
        return false;
    }

    // 🔧 🔹 1С ожидает МАССИВ объектов с полем "guid"
    $payload = json_encode([['guid' => $recordId]], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
    ...
    curl_setopt_array($ch, [
        ...
        CURLOPT_HTTPHEADER => [
            ...
            'Expect:',  // Отключаем 100-Continue для IIS
        ],
    ...
    if ($httpCode >= 200 && $httpCode < 300) {
        Logger::info('✅ Подтверждение отправлено в 1С', [
            'record_id' => $recordId, 
            'http_code' => $httpCode,
        ]);
        return true;
    } else {
        Logger::error('❌ Ошибка подтверждения в 1С', [
            'record_id' => $recordId,
            'http_code' => $httpCode,
            'curl_errno' => $errno,
            'curl_error' => $error,
